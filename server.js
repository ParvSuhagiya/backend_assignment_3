...
app.get("/states/high/highest-gdp", (req, res) => {

    if (states.length === 0) {
        return res.status(404).json({ message: "No states found" });
    }

    let highestState = states[0];

    for (let i = 1; i < states.length; i++) {
        if (states[i].gdp > highestState.gdp) {
            highestState = states[i];
        }
    }

    res.status(200).json(highestState);
});
...
app.put("/states/:id", (req, res) => {
    ...
    if (
        ...
        return res.status(400).json({
            message: "PUT requires complete state object"
        });
    }
...
app.put("/states/:id/population", (req, res) => {
    ...
    states[index].population = req.body.annualBudget;
...
app.patch("/states/:id", (req, res) => {
    const state = states.find(s => s.id === Number(req.params.id));
...
app.delete("/states/low-literacy/:percentage", (req, res) => {
  ...
  if (deletedCount === 0) {
    return res.status(404).json({
      message: "No states found below given literacy rate"
    });
  }
